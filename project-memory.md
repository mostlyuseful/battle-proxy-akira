...
- Context: `router.circuit-breaker` required Retry-After support and exponential backoff, but did not define base/max intervals or whether direct explicit model requests should be filtered.
- Decision: provider classification parses `Retry-After` as delta-seconds or HTTP-date and carries it on `provider.Error`. Router availability uses that exact time for rate-limit/exhaustion failures when present; otherwise it applies process-local exponential backoff starting at 30s, doubling per consecutive failure, capped at 5m. Successful requests reset the provider/model state. Synthetic routing skips candidates in an active exhaustion window; direct/explicit routes also return `no_available_model` while a configured candidate is temporarily unavailable.
- Rejected alternatives: using a fixed 30s window for every failure, because repeated exhaustion should back off more aggressively; making 401/403 authentication failures unavailable, because credential alternates are not modeled yet and the task notes say to be conservative for auth failures; skipping only synthetic aliases, because direct routing should not immediately hammer a provider/model already known to be exhausted.
- Affected area: provider error classification, static router candidate resolution, circuit-breaker state, and future admin/metrics state reporting.

### Server runtime shutdown defaults

- Context: `server.timeouts-shutdown` needed graceful shutdown and timeout configuration, but no config field exists for the shutdown grace period and the bootstrap server still used env-only address configuration.
- Decision: load runtime config from `LLM_PROXY_CONFIG` when present, preserve `LLM_PROXY_ADDR` as a development address override, and build the `http.Server` from `server.read_timeout_seconds`, `server.write_timeout_seconds`, and `server.idle_timeout_seconds` exactly as configured. Keep `write_timeout_seconds=0` as Go's zero `WriteTimeout` to support long-lived SSE streams. Handle SIGINT/SIGTERM by calling `http.Server.Shutdown` with a fixed 10s grace window, then force-close on timeout.
- Rejected alternatives: adding a new JSON shutdown timeout field in this task, because the current config schema/spec does not define one; setting a nonzero default write timeout, because that could break streaming by default.
- Affected area: CLI runtime startup, HTTP server lifecycle, streaming compatibility, and future operations/config schema work.

### Config reload design

- Context: `config.reload` required safe runtime reload without dropping in-flight requests or corrupting routing state. The spec lists config reload under operational hardening but does not specify a trigger mechanism, and the task notes say to prefer correctness over automatic filesystem watching. The prior `main.go` did not wire a router or providers at all, only health endpoints.
- Decision: introduce `internal/runtime.Manager` holding an `atomic.Pointer[Snapshot]` of `(config, *router.StaticRouter, providers)`. `Manager` implements `router.Router` (Resolve/MarkFailure/MarkSuccess) and exposes `Models` by delegating to the current snapshot, so the API handler built once at startup always uses the latest config. `Reload` re-runs the configured loader (which validates via `config.Load`/`Validate`), rebuilds providers and router, reconciles availability state for unchanged provider/model pairs via `AvailabilityTracker.Restore`/`StaticRouter.RestoreAvailability` (states for removed pairs are dropped), and atomically swaps the snapshot; any load/build/validation failure leaves the previous snapshot active. In-flight requests are not interrupted because they hold already-resolved `RouteCandidate` values and provider instances, and the swap is a single atomic pointer store. The explicit reload trigger is `SIGHUP`, handled by a separate `runReloadLoop` goroutine in `main.go` (disjoint from the SIGINT/SIGTERM shutdown loop); reload failures are logged but never terminate the proxy. `main.go` now wires the manager as chat router and model lister, plus client auth and logging built once from the initial config.
- Rejected alternatives: filesystem watching/inotify, because the task notes prefer an explicit trigger and it adds platform complexity; an admin HTTP reload endpoint, because admin endpoints are a separate task and out of scope; reloading client auth/logging/server timeouts live, because those are wired once at startup and changing addr/timeouts requires a restart — only routing/provider config is hot-reloaded here; reusing the old `AvailabilityTracker` directly across reloads, because that would retain state for removed provider/model pairs, so states are filtered to pairs still present in the new config.
- Affected area: runtime/config reload, router availability reconciliation, main server wiring, and future admin/metrics endpoints that may expose reload status.

### Responses API struct and IR mapping

- Context: `openai.responses-types` required Responses API request/response structs that normalize text and image inputs into IR, but the OpenAI Responses shape differs from Chat Completions (e.g. `input` is a string or array of EasyInputMessage items, image parts use `input_image` with `image_url`/`file_id`/`detail`, and sampling uses `max_output_tokens` rather than `max_tokens`/`max_completion_tokens`).
- Decision: in `internal/openai/responses_types.go`, model `ResponseRequest` with `Input` accepting either a plain string (single user message) or an array of message items; `instructions` becomes a `developer` role message in IR; `input_image` parts normalize to IR `input_image` content parts (reusing `ir.ContentTypeInputImage`); `max_output_tokens` maps to `SamplingParams.MaxCompletionTokens` only (Responses has no `max_tokens`, so `MaxTokens` stays nil). For output, model the assistant `message` item with `output_text`/`refusal` content parts; `ResponseFromIR` sets the item `status` from the IR finish reason (`length`/`max_output_tokens` -> `incomplete`, otherwise `completed`) and `ToIR` maps status back to a finish reason. Unknown input/output item types and unknown content part types are preserved verbatim in `Raw`/`Extra` rather than rejected at parse time, so forwarding is not lossy; structural validation (missing model/empty input/unsupported item type/image part without image_url or file_id) happens in `ToIR`.
- Rejected alternatives: mapping Responses `max_output_tokens` to both IR `MaxTokens` and `MaxCompletionTokens`, because the Chat Completions provider translation already reads `MaxCompletionTokens` and duplicating would risk double-setting; treating `input_image` parts as `image_url` IR parts, because the spec/IR distinguishes Chat Completions `image_url` from Responses `input_image` and preserving the distinction lets future provider adapters emit the correct part type; rejecting unknown item/content types at parse time, because the task notes say to reuse the IR and keep forwarding lossless, and strict rejection is enforced only when normalizing to IR.
- Affected area: Responses API edge parsing/translation, IR image content part type usage, and future Responses endpoint/provider-adapter tasks.

### Non-streaming Responses endpoint

- Context: `api.responses-nonstream` required a `POST /v1/responses` endpoint backed by the existing IR and provider routing, reusing chat provider calls and translating at the edge. The task notes that unsupported features must fail clearly rather than be silently ignored, and streaming Responses is out of scope (a separate task).
- Decision: add `internal/api/responses.go` with `RegisterResponsesRoutes` mirroring the chat handler (client auth, request-ID propagation, body-size limits, metadata logging, retryable pre-response fallback across candidates). Parse with `openaiapi.ParseResponseRequest` and normalize via `ResponseRequest.ToIR`; reject `stream: true` requests with `invalid_request` (400) so streaming is not silently degraded. Route the IR request through the shared `router.Router`, rewrite the successful provider response to the requested model alias via `RouteCandidate.RewriteResponse`, and emit the Responses shape via `openaiapi.ResponseFromIR`. Add a `WithResponsesRouter` server option that defaults to the chat router, so deployments can share one router or supply a dedicated one.
- Rejected alternatives: silently treating `stream: true` as non-streaming, because the task requires clear failures for unsupported features and clients expect SSE for streaming; building a separate Responses-specific router/provider path, because the IR and router are model-resolution agnostic and reuse is explicitly requested; returning Chat Completions JSON from `/v1/responses`, because the endpoint must return a Responses-compatible output shape.
- Affected area: API endpoint registration, Responses edge translation wiring, and future Responses streaming/translate tasks.

### Responses streaming translation

- Context: `api.responses-stream` required `stream=true` on `/v1/responses` with SSE translated from provider events. Providers stream Chat Completions-shaped `ir.Event`s (raw chunk JSON in `Text`/`Raw`), but Responses uses typed semantic SSE events. The Codex CLI is strict about the event sequence (it discards `output_text.delta` without a prior `content_part.added`).
- Decision: add `internal/openai/responses_stream.go` with a `ChatCompletionChunk` parser and a stateful `ResponsesStreamTranslator`. The translator parses each provider chunk to extract `choices[0].delta.content`, finish reason, and optional usage, and emits the lifecycle sequence `response.created` → `response.output_item.added` → `response.content_part.added` → `response.output_text.delta`* → `response.output_text.done` → `response.content_part.done` → `response.output_item.done` → `response.completed`. Role-only chunks produce no delta; unparseable chunks are skipped (not fatal). Response/item IDs are derived from the request ID (`req_` → `resp_`/`msg_`); the requested model alias is preserved in all response objects. Add `sse.WriteTypedEvent` for named `event:`+`data:` lines. The `streamResponsesRequest` handler reuses the chat streaming policy: retryable pre-stream errors fall back to the next candidate (JSON error if exhausted), no fallback after the first event, mid-stream failures emit an `error` SSE event then close without emitting closing lifecycle events. Usage from a final chunk is propagated into `response.completed`; finish reason `length` maps to `incomplete` status.
- Rejected alternatives: pass-through of raw Chat Completions chunks as Responses data, because Responses clients expect typed semantic events and `[DONE]` is Chat-Completions-specific; emitting the full Responses event taxonomy (refusal/tool/function-call/file/reasoning), because the task scopes to supported text output and full parity is out of scope; aborting the stream on an unparseable chunk, because one malformed chunk should not lose an otherwise healthy stream.
- Intentionally unsupported Responses event types (documented per task notes): refusal deltas, function_call/file_search/code_interpreter/computer/reasoning events, and annotation events. Only `output_text` deltas are translated.
- Affected area: Responses streaming edge translation, SSE helpers, and future Responses feature/tool-call streaming tasks.

### Metrics endpoint design

- Context: `ops.metrics-endpoint` required structured runtime counters (request counts by endpoint/status class, error counts by internal code, latency summaries) exposed as JSON, without a metrics dependency. The task notes call for stdlib sync primitives and low overhead; Prometheus exposition and distributed metrics are out of scope.
- Decision: add `internal/metrics` with a mutex-protected `Collector` holding request counts keyed by `(endpoint, statusClass)`, error counts by internal error code, per-bucket `latencySummary` (count/sum/min/max/mean) plus a 128-sample ring buffer of recent durations for nearest-rank p50/p95/p99. Expose `GET /metrics` returning a sorted, deterministic JSON `Snapshot` (requests, errors, latency, totals, started_at/generated_at). Wire via `api.WithMetrics`, an outer `metricsMiddleware` using a `statusRecorder` to capture the status code and record endpoint/status-class/latency per request, and a context-stored collector so `writeLoggedOpenAIError` (the single proxy-error choke point) records internal error codes by reading the collector from the request context. `main.go` constructs one collector for the process lifetime.
- Rejected alternatives: a Prometheus text-format endpoint, because it is explicitly out of scope and would require a format/dependency decision; recording metrics inside each handler inline, because that is high-touch and error-prone versus one outer middleware for endpoint/status/latency plus the existing error choke point for codes; per-request full-latency histograms, because the nearest-rank ring buffer keeps memory bounded while still offering percentile estimates; histogram buckets à la Prometheus, because MVP only needs aggregate summaries and recent percentiles.
- Affected area: operations observability, request lifecycle middleware, and future admin/metrics exposition (Prometheus, per-route, per-model dimensions).
